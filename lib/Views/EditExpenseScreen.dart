  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _EditTransactionForm(
        ...
        deleteLabel: 'מחק הוצאה',
        initialAmount: amount,
        initialCategory: category,
        initialDate: date,
        initialNote: note,
        categories: const ['מזון', 'תחבורה', 'חשבונות', 'קניות', 'בילוי', 'חינוך', 'ביטוח', 'בריאות', 'אחר'],
          ...
          try {
            ...
            if (context.mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('ההוצאה עודכנה בהצלחה ✓')));
            }
          } catch (e) {
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
        ...
        onDelete: () async {
          try {
            ...
            if (context.mounted) {
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('ההוצאה נמחקה ✓')));
            }
          } catch (e) {
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
